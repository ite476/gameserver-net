...
## 7) Merge Commit 메시지 규칙

릴리스/핫픽스 PR은 **Merge commit**을 사용하며, 커스텀 커밋 메시지를 작성합니다.
...
관련 이슈: Resolves #X, Resolves #Y, Resolves #Z
```

### 예시

```
release(0.2.0): 정규 업데이트

마일스톤 0.2.0 (Architecture Decision Records) 완료
- ADR 문서 3개 추가 (매치메이킹, 실시간 채팅, 매치 세션 & MMR)
- 프로젝트 기초 구조 세팅
- PR 템플릿 및 컨벤션 업데이트

관련 이슈: Resolves #13, Resolves #14, Resolves #15, Resolves #27, Resolves #28, Resolves #29, Resolves #31, Resolves #32
참고 이슈: related to: #10 (참고한 이슈, 이슈를 닫지 않음)
```

### 작성 가이드

* **제목**: PR 제목과 동일하게 작성
* **본문 첫 줄**: 마일스톤 정보 또는 릴리스 요약 (1줄)
* **주요 변경사항**: 불릿 포인트로 핵심 변경사항 나열 (3-5개 권장)
* **관련 이슈**: 
  * 완료한 이슈: `Resolves #<이슈번호>` (실제로 구현 및 완료한 이슈만)
  * 참고 이슈: `related to: #<이슈번호>` (연관/참조만 하는 경우, 이슈를 닫지 않음)

### 이유

* 릴리스 정보가 히스토리에 명확히 기록됨
* `git log`에서 릴리스 내용을 바로 확인 가능
* 태그와 함께 사용 시 일관성 유지
* 롤백 시 Merge commit 하나만 revert하면 전체 릴리스 제거 가능
