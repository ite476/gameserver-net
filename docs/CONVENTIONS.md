...
* Q. 실수로 잘못된 브랜치에 머지한 PR이나 리버트 PR도 이슈를 만들어야 하나요? → A. **아니요, 새 이슈 생성 불필요**합니다. 이미 처리 완료된 작업이므로 PR 히스토리로 충분히 추적 가능합니다. 다만, 기존 이슈에서 파생된 작업이라면 PR 본문에 `related to: #<이슈번호>`를 추가하여 기존 이슈에 연결하는 것을 권장합니다.
